...
def invoke_endpoint(endpoint_name):
    ...
    try:
        ...
        response = sm_runtime_client.invoke_endpoint(
            EndpointName=endpoint_name, Body=sample, ContentType="text/csv"
        )
        score = eval(response["Body"].read())[0]
        msg = f"The predictive score for fraud is {score}."
        logger.info(msg)

        return {"endpoint_name": endpoint_name, "success": True}
    except ClientError:
        return {"endpoint_name": endpoint_name, "success": False}
