...
if __name__ == "__main__":
    ...
    test_identity = pd.read_csv(f"{base_dir}/raw_data/prediction/test_identity.csv")
    test_transaction = pd.read_csv(
        f"{base_dir}/raw_data/prediction/test_transaction.csv"
    )
    ...
    arr_re_train = np.concatenate(
        (df_y_re_train.values.reshape(-1, 1), X_re_train), axis=1
    )
    arr_re_valid = np.concatenate(
